...
===================
  ...
  1. `axcf.m` - MATLAB function for determining covariance matrices (ACF, AXCF)
  2. `saxcf.m` - MATLAB function for determining stationarized covariance matrices (SACF, SAXCF)
  3. `example.m` - MATLAB example using these two functions
  4. `src/approxc.f90` - Fortran 90 module containing the calculation subroutines ACF, AXCF, SACF, SAXCF
  5. `src/nr.for` - Fortran 77 subroutine four1 from Numerical Recipes
  6. `src/example.f90` - Fortran 90 main program for calculating covariances from data in `example_data.txt`
  7. `Makefile` - Compilation script for the Fortran part
  8. `run_example_plot.m` - Matlab script to load and plot matrices generated by the Fortran code
...
===================
  ...
  MATLAB:
  1. Open MATLAB
  2. Run the main script: `example.m` to visualize results from MATLAB
  
  Fortran:
  1. Compile Fortran codes
  2. Run the program: `run_example` in the project root directory
  3. Run the MATLAB script: `run_example_plot.m` to visualize results from Fortran
